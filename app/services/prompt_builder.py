...
def _fallback_system_prompt(input_mode, conversation_style):
    ...
    return (
        ...
        "- final_output.reason 必須同時說明：使用者需求 + 服務特色如何對應。\n"
        "- 一律使用繁體中文。\n"
    )
...
def _task_button_step_instruction(current_turn):
    if current_turn <= 1:
        return (
            ...
            "選項可用：剪髮、染髮、補染、漂髮、燙髮、護髮、頭皮護理、組合服務。\n"
            "若使用者選不確定，下一輪改問分類題（改變髮色 / 改變髮型 / 改善髮質頭皮 / 修整）。\n"
        )
    if current_turn == 2:
        return (
...
def _phase_instruction(input_mode, conversation_style, current_turn):
    ...
    if conversation_style == "task":
        if current_turn <= 2:
            return (
                "本輪 task-led 要求：先確認服務方向（剪髮/染髮/補染/漂髮/燙髮/護髮/頭皮/組合）。"
                "只問一個能直接推進決策的封閉式問題。\n"
            )
        if current_turn <= 3:
            return (
